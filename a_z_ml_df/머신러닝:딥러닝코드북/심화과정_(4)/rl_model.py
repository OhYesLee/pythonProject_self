...
import gym
from custom_envs.bipedal_walker_ray import AugmentBipedalWalkerHardcoreSmallLegs
from custom_envs.bipedal_walker_ray import AugmentBipedalWalkerHardcore
from custom_envs.bipedal_walker_ray import AugmentBipedalWalkerSmallLegs
from custom_envs.bipedal_walker_ray import AugmentBipedalWalkerTallLegs
from custom_envs.bipedal_walker_ray import AugmentBipedalWalker

render_mode = False #turn off gui
...
class Model:
  ...
  def __init__(self, game):
    ...
    for shape in self.shapes:
      self.weight.append(np.zeros(shape=shape)) #define model weight
      self.bias.append(np.zeros(shape=shape[1]))#define model bias
      self.param_count += (np.product(shape) + shape[1]) #parameter count
    ...
    self.body_param = np.ones(self.body_size) #define body parameter
  ...
  def make_env(self, seed=-1, render_mode=False):
    ...
    gym_env = gym.make("custom_env-v0")
    self.env = make_env(self.env_name, seed=seed, render_mode=render_mode)

  def augment_env(self):
    self.env.augment_env(self.body_param)

  def get_action(self, x, t=0, mean_mode=False):#take action according to weight
  ...
  def set_model_params(self, model_params): #set model parameter
    pointer = 0
...
def simulate(config,model,select_env, train_mode=False, render_mode=True, num_episode=5, seed=-1, max_len=-1):
  agent = pg.PGTrainer(config=config, env=select_env)
  reward_list = []
  t_list = []
...
#  game = config.games[gamename]
  ...
  ray.init()
  configs = pg.DEFAULT_CONFIG.copy()
#config["num_gpus"] = 0.8
#config["num_workers"] = 3
#config["eager"] = False
#config["monitor"]=True
  select_env = "custom_env-v0" 
  register_env(select_env, lambda config: AugmentBipedalWalkerSmallLegs())
  game = config.games["augmentbipedsmalllegs"]
  register(
    id="custom_env-v0",
    entry_point="custom_envs.bipedal_walker_ray:AugmentBipedalWalkerSmallLegs",)
  gym_env = gym.make("custom_env-v0")
  ...
  if final_mode:
    ...
    for i in range(100):
      reward, steps_taken = simulate(configs, model, select_env, rain_mode=False, render_mode=False, num_episode=1)
      total_reward += reward[0]
    ...
    while(5):
      reward, steps_taken = simulate(configs, model, select_env, train_mode=False, render_mode=render_mode, num_episode=1)
