...
import sys
import config
from rl_model import make_model, simulate
...
def initialize_settings(game_gym,sigma_init=0.1, sigma_decay=0.9999):
  global population, filebase, game, model, num_params, es, PRECISION, SOLUTION_PACKET_SIZE, RESULT_PACKET_SIZE
  population = num_worker * num_worker_trial
  filebase = 'saved_model/'+gamename+'.'+optimizer+'.'+str(num_episode)+'.'+str(population)
  model = make_model(game_gym)
...
def worker(config, select_env_s, weights, seed, train_mode_int=1, max_len=-1):

  train_mode = (train_mode_int == 1)
  model.set_model_params(weights)
  reward_list, t_list = simulate(config,model,select_env_s,
    train_mode=train_mode, render_mode=False, num_episode=num_episode, seed=seed, max_len=max_len) #render_mode=True if you want turn on gui
  if batch_mode == 'min':
...
def slave(config, select_env_s):
  ...
  while 1:
    ...
    for solution in solutions:
      ...
      jobidx = int(jobidx)
      seed = int(seed)
      fitness, timesteps = worker(config, select_env_s, weights, seed, train_mode, max_len)
...
def master(config, select_env_m):
  ...
  while True:
    ...
    if t%100==0:
      with open(filebase+"_"+str(t)+"_"+str(avg_reward)+".json", 'wt') as out:
        res = json.dump([np.array(es.current_param()).round(4).tolist()], out, sort_keys=True, indent=2, separators=(',', ': '))
...
#config["monitor"]=True
  ...
  initialize_settings(game,args.sigma_init, args.sigma_decay)
  print("process", rank, "out of total ", comm.Get_size(), "started")
  if (rank == 0):
    master(configs, select_env)
  else:
    slave(configs, select_env)
